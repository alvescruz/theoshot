...
impl SelectionApp {
    ...
    fn get_canvas_rect(&self, ctx: &egui::Context) -> egui::Rect {
        ...
        egui::Rect::from_center_size(screen_rect.center() - egui::vec2(0.0, 30.0), egui::vec2(canvas_w, canvas_h))
    }
    ...
    fn render_to_image(&self, canvas_rect: egui::Rect) -> RgbaImage {
        ...
        let font_data = std::fs::read("/usr/share/fonts/noto/NotoSans-Bold.ttf").ok().or_else(
        ...
        for shape in &self.shapes {
            ...
            match shape.tool {
                ...
                Tool::Blur if shape.points.len() >= 2 => {
                    ...
                    let (rx, ry, rw, rh) =
                        (p1.0.min(p2.0), p1.1.min(p2.1), p1.0.abs_diff(p2.0), p1.1.abs_diff(p2.1));
                    if rw > 0 && rh > 0 {
    ...
    fn styled_button(
        ...
        let button = egui::Button::new(text_widget)
            .fill(if selected {
                self.stroke_color // Usa a cor da paleta no fundo
            } else {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.texture.is_none() && self.image_data.is_some() {
            ...
            self.texture = Some(ctx.load_texture(
                "bg",
                egui::ColorImage { size: [self.width as usize, self.height as usize], pixels },
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |_ui| {
                let window_frame = egui::Frame::default()
                    ...
                    .rounding(egui::Rounding::ZERO) // Borda reta conforme solicitado
                    .shadow(egui::Shadow {
                        blur: 40.0,
                ...
                egui::Area::new(egui::Id::new("main_view"))
                    ...
                    .show(ctx, |ui| {
                        ...
                        window_frame.show(ui, |ui| {
                            ...
                            let (res, painter) = ui.allocate_painter(canvas_rect.size(), egui::Sense::drag());
                            if let Some(tex) = &self.texture {
                                painter.image(tex.id(), canvas_rect, egui::Rect::from_min_max(egui::pos2(0.0, 0.0), egui::pos2(1.0, 1.0)), egui::Color32::WHITE);
                            }

                            for shape in &self.shapes { self.draw_shape_ui(&painter, shape, false, canvas_rect, ctx); }
                            if let Some(shape) = &self.current_shape { self.draw_shape_ui(&painter, shape, true, canvas_rect, ctx); }
                            ...
                            if res.dragged() {
                                if let (Some(p), Some(shape)) = (pos, &mut self.current_shape) {
                                    if shape.tool == Tool::Pen { shape.points.push(p); } else { shape.points[1] = p; }
                                }
                            }
                            if res.drag_stopped() {
                            ...
                            if self.current_tool == Tool::Text {
                                if let Some(shape) = &mut self.current_shape {
                                    ...
                                    if finished { if let Some(s) = self.current_shape.take() { self.shapes.push(s); } }
                                }
                            }
                        });
                            ...
                            egui::Frame::default()
                                ...
                                .show(ui, |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.scope(|ui| {
                                            ui.visuals_mut().widgets.inactive.rounding = egui::Rounding::same(18.0);
                                            ui.visuals_mut().widgets.hovered.rounding = egui::Rounding::same(18.0);
                                            ui.visuals_mut().widgets.active.rounding = egui::Rounding::same(18.0);
                                        ...
                                        for (t, icon, name) in tools {
                                            ...
                                            // Calcula cor de contraste para o ícone sobre o fundo colorido
                                            let icon_color = if is_active {
                                                if self.stroke_color.r() as u32 + self.stroke_color.g() as u32 + self.stroke_color.b() as u32 > 382 {
                                                    egui::Color32::BLACK
                                                } else {
                                                    egui::Color32::WHITE
                                                }
                                            ...
                                            if self.styled_button(ui, egui::RichText::new(icon).size(20.0).color(icon_color), is_active)
                                        ...
                                        ui.separator();
                                        if self.styled_button(ui, egui::RichText::new(icons::ARROW_U_UP_LEFT).size(18.0), false).on_hover_text("Desfazer").clicked() {
                                            if let Some(s) = self.shapes.pop() { self.redo_stack.push(s); }
                                        }
                                        if self.styled_button(ui, egui::RichText::new(icons::ARROW_U_UP_RIGHT).size(18.0), false).on_hover_text("Refazer").clicked() {
                                            if let Some(s) = self.redo_stack.pop() { self.shapes.push(s); }
                                        }

                                        ui.separator();
                                        if self.styled_button(ui, egui::RichText::new(icons::COPY_SIMPLE).size(20.0), false).on_hover_text("Copiar (Ctrl+C)").clicked() {
                                            self.copy_action(canvas_rect);
                                        }

                                        if self.action_button(ui, egui::RichText::new(icons::FLOPPY_DISK).size(20.0).color(egui::Color32::WHITE), egui::Color32::from_rgb(60, 130, 255))
                                            .on_hover_text("Salvar (Ctrl+S)").clicked() {
                                            self.save_action(canvas_rect);
                                        }
                                    });
                                });
